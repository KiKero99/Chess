...
export class BoardComponent {
  ...
  isHighlighted(square: number, index: number): boolean {
  ...
  onClickSquare(square: number, index: number){
    if (this.availableMovesManager.isAPieceSelected && this.isHighlighted(square, index)) {
    ...
    if (this.playerService.color !== GameService.getPieceColor(square)) return;
